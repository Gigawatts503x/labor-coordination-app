...
export function DataStoreProvider({ children }) {
  ...
  // NOTE: Backend only supports /api/events/{eventId}/requirements
  // Generic /api/requirements endpoint doesn't exist, so we skip it for now
  const fetchRequirements = useCallback(async (eventId = null) => {
    try {
      if (!eventId) {
        // Backend doesn't have generic requirements endpoint, skip
        console.warn('fetchRequirements: eventId not provided, skipping fetch');
        return [];
      }
  ...
  const syncAllData = useCallback(async () => {
    ...
    try {
      await Promise.all([
        ...
        fetchAssignments(),
        fetchTechnicians(),
        // SKIP fetchRequirements on mount - backend endpoint doesn't support generic fetch
        // fetchRequirements(),
      ]);
      dataEventEmitter.emit(DATA_EVENTS.SCHEDULE_SYNC, null);
    } catch (error) {
  ...
  }, [fetchEvents, fetchAssignments, fetchTechnicians]);
  ...
  const addTechnician = useCallback(
    async (technicianData) => {
      try {
        ...
        dispatch({ type: 'ADD_TECHNICIAN', payload: newTechnician });
        dataEventEmitter.emit(DATA_EVENTS.TECHNICIANS_UPDATED, newTechnician);
  ...
  const deleteTechnician = useCallback(
    async (technicianId) => {
      try {
        ...
        dataEventEmitter.emit(DATA_EVENTS.TECHNICIANS_UPDATED, null);
        return true;
      } catch (error) {
